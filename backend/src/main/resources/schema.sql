...
CREATE TABLE IF NOT EXISTS app_user (
    ...
    auth_source      VARCHAR(30)  NOT NULL DEFAULT 'LOCAL' COMMENT 'LOCAL/WECOM/SCHOOL_SSO',
    external_subject VARCHAR(200) COMMENT '学校统一身份认证返回的稳定用户标识',
    wecom_user_id    VARCHAR(200) COMMENT '企业微信成员UserId，用于应用消息通知',
...
CREATE TABLE IF NOT EXISTS activity (
    ...
    check_in_mode    VARCHAR(20)   NOT NULL DEFAULT 'QR' COMMENT 'QR/LOCATION/MANUAL/NONE',
    check_in_open    BOOLEAN       NOT NULL DEFAULT FALSE COMMENT '现场签到是否开放',
    check_in_code    VARCHAR(10)   COMMENT '现场动态签到码',
    check_in_opened_at DATETIME    COMMENT '签到开启时间',
    check_in_latitude DOUBLE       COMMENT '定位签到中心纬度',
    check_in_longitude DOUBLE      COMMENT '定位签到中心经度',
    check_in_radius_meters INT     DEFAULT 100 COMMENT '定位签到有效半径（米）',
    check_in_location_accuracy_meters DOUBLE COMMENT '教师设置签到点时的定位精度（米）',
...
CREATE TABLE IF NOT EXISTS activity_registration (
    ...
    check_in_method VARCHAR(20) COMMENT 'SELF_CODE/SELF_LOCATION/MANUAL',
    checked_in_by VARCHAR(100),
    check_in_distance_meters INT COMMENT '定位签到时距中心点距离（米）',
    check_in_accuracy_meters DOUBLE COMMENT '学生签到时的定位精度（米）',
