  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex w-full items-center justify-between rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-left sm:block sm:w-auto sm:px-5 sm:text-right">
      ...
      <section className="mt-5 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm sm:mt-6 sm:rounded-[24px]">
        ...
        {loading ? (
          ...
          <div className="grid gap-4 p-4 lg:grid-cols-2 sm:p-5">
            {tasks.map((activity) => (
              <article key={activity.id} className="rounded-2xl border border-stone-200 p-4 transition hover:border-indigo-200 hover:shadow-sm sm:p-5">
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <button type="button" onClick={() => openPreview(activity)} className="min-h-10 rounded-xl border border-stone-200 px-2 py-2 text-xs font-bold text-stone-600 hover:bg-stone-50 sm:px-4">查看材料</button>
                  <button type="button" onClick={() => setDecision({ activity, type: 'reject' })} className="min-h-10 rounded-xl border border-red-200 px-2 py-2 text-xs font-bold text-red-600 hover:bg-red-50 sm:px-4">驳回</button>
                  <button type="button" onClick={() => setDecision({ activity, type: 'approve' })} className="min-h-10 rounded-xl bg-indigo-600 px-2 py-2 text-xs font-bold text-white hover:bg-indigo-700 sm:px-4">通过</button>
                </div>
              </article>
            ))}
          </div>
...
function ApprovalFlow({ activeStage }) {
  ...
  return (
    <div className="mt-6 grid gap-2 rounded-2xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-white p-3 sm:mt-7 sm:grid-cols-3 sm:rounded-[22px] sm:p-5">
...
function DecisionDialog({ decision, roleLabel, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-stone-950/55 p-0 backdrop-blur-sm sm:grid sm:place-items-center sm:p-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="w-full max-w-lg rounded-t-3xl bg-white p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl sm:rounded-3xl sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-xl px-4 py-2.5 text-sm font-semibold text-stone-500 hover:bg-stone-100">取消</button><button type="button" disabled={busy || (rejecting && !comment.trim())} onClick={submit} className={`min-h-11 rounded-xl px-5 py-2.5 text-sm font-bold text-white disabled:opacity-40 ${rejecting ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>{busy ? '正在处理…' : (rejecting ? '确认驳回' : '确认通过')}</button></div>
      </div>
    </div>
  )
}
...
function PreviewDialog({ activity, history, onClose }) {
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-950/55 p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] backdrop-blur-sm sm:p-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="mx-auto my-2 max-w-4xl sm:my-10">
